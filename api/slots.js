...
export default async function handler(req, res) {
    ...
    `;
        ...
        return res.status(200).json({
            booked: booked.map(r => r.booking_time),
            blocked: blocked.map(r => r.slot_time)
        });
    }
    ...
    if (req.method === 'POST') {
        ...
        if (!date || !time || !action) {
            return res.status(400).json({ error: 'date, time, action required' });
        }
      ...
      `;
            ...
            await sql`DELETE FROM blocked_slots WHERE slot_date = ${date} AND slot_time = ${time}`;
        } else {
